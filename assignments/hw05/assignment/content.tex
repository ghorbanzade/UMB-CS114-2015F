...
Note that words ``\textit{They}'' and ``\textit{they}'' are considered as one word. In addition, the program is not sensitive to punctuations and regards the words   ``\textit{fence}'', ``\textit{fence,}'' and ``\textit{fence.}'' as three occurences of the same word.

A template for how to open, read and close files in Java is provided in the file \texttt{FileHandleSample.java} that accompanies this assignment.
...
Write a program \texttt{SnakeGame.java} that implements a simplified version of the fameous snake arcade game where user controls movement of a snake towards the food source within a matrix of size $5 \times 10$.
